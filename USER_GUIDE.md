...
4.  **开通 API 权限**：
    *   在"权限管理"页面，搜索并开通以下权限：
    ...
    | 权限名称（中文搜索） | Scope ID | 说明 |
    |:---|:---|:---|
    | 获取与发送单聊、群组消息 | `im:message` | 消息读写（包含发送） |
    | 获取群组信息 | `im:chat:readonly` | 获取群聊列表 |
    | 接收消息 | `im:message.receive_v1` | 接收群聊消息事件 |

5.  **配置事件订阅**：
    *   在"事件与回调" -> "事件配置"页面，添加 `im.message.receive_v1`（接收消息）事件。
    *   确认事件订阅方式为**长连接（WebSocket）**，而非 HTTP 回调。
6.  **发布应用**：完成以上配置后，在"版本管理与发布"页面创建并发布一个新版本。**每次修改权限或事件订阅后都需要重新发布**。
7.  **邀请入群**：将创建好的机器人邀请到您希望它参与讨论的飞书群聊中。
...
以下是在实际飞书群聊环境中的验证结果，使用 3 个智谱 API Key 配置了 3 个 Bot：
...
|:---|:---|
| 模型 | glm-4.5-flash |
| Bot 数量 | 3（分析师小智、创意师小慧、总结师小明） |
| 飞书连接方式 | WebSocket 长连接 |
| 群聊名称 | CARLE |
...
**分析师小智**：你好！我是分析师小智，很高兴为你提供专业的分析服务。有什么问题需要我帮忙分析吗？

**创意师小慧**：你好！很高兴见到你！我是创意师小慧，随时准备和你一起头脑风暴，激发灵感火花！

**总结师小明**：你好！很高兴为您服务，我是总结师小明，擅长归纳讨论要点和形成清晰结论。
